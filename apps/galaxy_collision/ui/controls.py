"""
Control Panel mit Unicode-Unterstützung
"""
...
def _font():
    """Holt den aktuellen Font-Namen (dynamisch)."""
    return get_font_name()


class ParamSlider(BoxLayout):
    """Parameter-Slider mit Unicode-Labels"""

    def __init__(self, label, min_v, max_v, default, step, fmt="{:.1f}", callback=None, **kw):
        super().__init__(**kw)
        ...
        self.slider = Slider(
            min=min_v, max=max_v, value=default, step=step,
            size_hint_y=0.55,
            value_track=True,
...
class ControlPanel(BoxLayout):
    """Control Panel mit Unicode-Symbolen"""
    ...
    def _lbl(self, **kwargs):
        """Erstellt Label mit korrekter Schriftart."""
        kwargs['font_name'] = _font()
        return Label(**kwargs)

    def _btn(self, **kwargs):
        """Erstellt Button mit korrekter Schriftart."""
        kwargs['font_name'] = _font()
    ...
    def _build_ui(self):
        ...
        zoom_label = self._lbl(text="Zoom", font_size='12sp')
        ...
        self._add_slider(params, "Partikel", 5000, 30000, 20000, 5000, "{:.0f}", "total_particles")
        self._add_slider(params, "M-Verhältnis", 0.2, 5.0, 1.0, 0.2, "{:.1f}", "mass_ratio")
        self._add_slider(params, "Gesamtmasse", 1.0, 4.0, 2.0, 0.5, "{:.1f}", "total_mass")
        ...
        self._add_slider(params, "Abstand", 30.0, 80.0, 50.0, 5.0, "{:.0f}", "initial_distance")
        self._add_slider(params, "v/v_escape", 0.2, 0.9, 0.5, 0.1, "{:.1f}", "velocity_factor")
        self._add_slider(params, "Impact", 0.0, 0.7, 0.3, 0.1, "{:.1f}", "impact_parameter")
        self._add_slider(params, "Neigung", 0.0, 90.0, 30.0, 10.0, "{:.0f}" + S.DEGREE, "inclination")
        ...
        self._add_slider(params, "Reibung", 0.0, 0.05, 0.01, 0.005, "{:.3f}", "dynamic_friction")
        ...
        # Status-Label (mehr Platz für Info)
    ...
    def _zoom(self, delta):
        ...
        if app and hasattr(app, 'view') and app.view:
            cam = app.view.camera
            cam.distance = max(cam.min_distance, min(cam.max_distance, cam.distance + delta))
    ...
    def update_stats(self, stats):
        ...
        if galaxy_count == 1:
            # Einzelgalaxie-Modus
            self.stats_lbl.color = (0.4, 0.7, 0.9, 1)
            self.stats_lbl.text = (
                f"t={t:.1f} | N={n}\n"
                f"<r>={mean_radius:.2f}\n"
                f"M={ma:.2f}"
            ...
            # Verschmolzen
            self.stats_lbl.color = (0.8, 0.6, 0.9, 1)
            self.stats_lbl.text = (
                f"t={t:.1f} | N={n}\n"
            ...
            # Zwei Galaxien
            self.stats_lbl.color = (0.5, 0.5, 0.5, 1)
            self.stats_lbl.text = (
                f"t={t:.1f} | d={d:.1f} | N={n}\n"
                f"Passagen: {passages}\n"
                f"M: {ma:.2f}/{mb:.2f}"
